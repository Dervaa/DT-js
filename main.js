...
var solveDifferentialEquation = (nextTimestamp, difficulty) => {

  nextTimestamp = Math.round(nextTimestamp)
  var t = nextTimestamp; // время
  var h = Math.log(difficulty); // параметр сложности
  var x = Math.sin(t*h / 2) * Math.cos(t*h / 2);
  x *= 1e6; // Умножаем на 1 000 000 для получения целого числа с шестью разрядами
  x = Math.round(x); // Округляем до ближайшего целого числа
  return x;
};
var mineBlock = (blockData) => {
  ...
  while (true) {
    ...
    var solution = solveDifferentialEquation(nextTimestamp, difficulty);

    if (nextHash.toString().substring(0, difficulty) === solution.toString().substring(0, difficulty)) {
      break;
    }
